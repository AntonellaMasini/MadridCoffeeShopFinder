...
from CoffeeShopApp.models import *
import csv
...
csv_path = Path(__file__).parent.parent / 'csvs'/ 'coffeeshops.csv'  #dynamic path construvtion, good for portability!
    ...
    if existing_records == 0:
        ...
        with open(csv_path, "r") as file:
            ...
            for row in reader: #row is a dict
                for key, value in row.items():  # Convert 'TRUE'/'FALSE' to Python booleans (True/False)
                    if value == 'TRUE':
                        row[key] = True
                    elif value == 'FALSE':
                        row[key] = False
